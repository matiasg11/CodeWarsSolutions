...
sumDig_nthTerm(10, [1, 2, 3], 15) ----> 10 # 37 is the 15-th term, and 3 +
*/

function sumDigNthTerm(initval, patternl, nthterm) {
    
    let p = patternl.length  //This depends on what we have to sum to each term
    
    //nthterm will always be lower or equal than p
    
    for(let i=0; i<nthterm-1; i++){
      initval = initval + patternl[i%p]
    }
  
  let values = initval.toString().split('')
  let nums = values.map(num => Number(num))
